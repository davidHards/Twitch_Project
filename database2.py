...
def streamsToList1(self):
    ...
    s1Data2 = list(s1Data)
    dataSet = set(tuple(x) for x in s1Data2)
    s1Data = [ list (x) for x in dataSet ]

    return createStreamDB1(s1Data)
...
def createStreamDB1(self):
    ...
    for x in streams:
        tupleList = tuple(x)
        sql = "INSERT INTO streams2 (id, user_id, user_name, game_id, community_ids, type, title, viewer_count, started_at, language, thumbnail_url, tag_ids, time_stamp) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        val = tupleList
        try:
            mycursor.execute(sql, val)
            mydb.commit()
