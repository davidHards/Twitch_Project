...
def getFileNames():
    ...
    os.chdir("F://Twitch_Data_Nov-Feb")
...
def createStreamDB(self):
    ...
    for x in sData:
        tupleList = tuple(x)
        sql = "INSERT INTO complete_data (id, user_id, user_name, game_id, community_ids, type, title, viewer_count, started_at, language, thumbnail_url, tag_ids, time_stamp) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        val = tupleList
        try:
